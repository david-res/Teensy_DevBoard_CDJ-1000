...
uint16_t sampleRate = 44100; // Default sample rate, will be updated when track is loaded
...
static void overviewWaveformClick(lv_event_t *e) {
    ...
    if (code == LV_EVENT_PRESSED || code == LV_EVENT_CLICKED || code == LV_EVENT_PRESSING) {
        ...
        if (x != last_x) {
            last_x = x;
            SEEK_AUDIOFRAME(samplesPerLine * (((x-40) * all_long) / PREVIEW_WAVEFORM_WIDTH-1));
        }
    }
}
...
void create_bottom_container(Track * track) {
    ...
    lv_obj_set_size(bottom_container, chartWidth, middleContainerPos);
    ...
    lv_obj_set_pos(static_wave_container, 40, 80);
...
void dj_ui_init(Track * track) {
   ...
   // lv_obj_set_style_border_color(main_screen, lv_color_hex(0xFFFFFF), 0);
    ...
    samplesPerLine = track->sample_rate/150;
